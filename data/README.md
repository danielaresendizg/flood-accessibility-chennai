# Data Access (Chennai Flood Accessibility)

This repository does not include large geospatial datasets. Use the Google Drive folder described in [`../DATA_SOURCES.md`](../DATA_SOURCES.md).

## Download

- **Google Drive folder (public):** _TODO_

## Samples

If you want to add small examples for reproducibility, place them under:

- `data/sample/`

Keep samples lightweight (CSV/GeoJSON) and avoid uploading large rasters or geopackages.

